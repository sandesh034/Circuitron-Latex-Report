...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{>{\bfseries}l>{\bfseries}X>{\raggedright\arraybackslash}X}
        \toprule
        \textbf{HTTP Method} & \textbf{Endpoint} & \textbf{Functionality} \\
        \midrule
        GET & /api/v1/ & Returns basic \gls{api} information and available documentation links (health check) \\
        POST & /api/v1/simulation/simulate & Submits a \gls{spice} netlist and initiates a circuit simulation \\
        POST & /api/v1/simulation/validate & Validates the syntax and structure of a \gls{spice} netlist without running simulation \\
        ...
        GET & /api/v1/docs & Provides interactive Swagger \gls{ui} for \gls{api} exploration \\
        GET & /api/v1/redoc & Provides ReDoc-based \gls{api} documentation \\
        GET & /api/v1/openapi.json & Returns OpenAPI specification in \gls{json} format \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering [Figure Placeholder: Interaction Diagram for the Web App showing Frontend (React.js) communicating with Backend API (FastAPI) through REST endpoints, including request-response flows for simulation submission, status checking, and result retrieval]}}
    \caption{Interaction Diagram for the Web Application}
