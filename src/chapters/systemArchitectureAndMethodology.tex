...
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering [Block Diagram Placeholder: Preprocessing $\rightarrow$ Detection $\rightarrow$ OCR $\rightarrow$ Wire Detection $\rightarrow$ Netlist $\rightarrow$ Simulation/Schematic]}}
...
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering [Figure Placeholder: YOLOv7 Architecture showing Backbone, Neck, and Head modules]}}
...
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering [Figure Placeholder: Text Detection and Recognition Pipeline showing IMAGE $\rightarrow$ YOLO Bounding Box $\rightarrow$ Sort and Crop $\rightarrow$ Resize \& Normalize $\rightarrow$ VGG Feature Extractor $\rightarrow$ BiLSTM Layers $\rightarrow$ Linear Projection $\rightarrow$ Greedy Decoder $\rightarrow$ Post-Process Output]}}
...
For classification purposes, the angle is converted to a positive representation in degrees using the transformation $\tilde{\theta} = (180\theta / \pi) \mod 360$, yielding values in the range $[0°, 360°)$.
...
\begin{equation}
\Theta_H = [0°, 18°] \cup [162°, 198°] \cup [342°, 360°]
\label{eq:horizontal-region}
\end{equation}

This region captures angles within $\pm 18°$ of the positive $x$-axis ($0°$ and $360°$) and within $\pm 18°$ of the negative $x$-axis ($180°$).

\textbf{Vertical Acceptance Region:} The set of angles classified as vertical is defined as:

\begin{equation}
\Theta_V = [72°, 108°] \cup [252°, 288°]
\label{eq:vertical-region}
\end{equation}

This region captures angles within $\pm 18°$ of the positive $y$-axis ($90°$) and within $\pm 18°$ of the negative $y$-axis ($270°$).
...
\begin{equation}
\Theta_D = [0°, 360°) \setminus (\Theta_H \cup \Theta_V)
...
The classification function $\mathcal{O} : [0°, 360°) \to \{H, V, \varnothing\}$ maps each angle to its corresponding orientation category. The total angular coverage of valid classifications is $(|\Theta_H| + |\Theta_V|)/360° = 144°/360° = 40\%$, indicating that 40\% of angular space is accepted as orthogonal while 60\% is rejected as diagonal.
...
\textbf{Projection Error Bound:} For a junction pair with orientation angle $\tilde{\theta} \in [0°, \epsilon]$ and inter-junction distance $L = \|\mathbf{d}^{\leftrightarrow}\|$, the maximum perpendicular displacement of any endpoint from its original position is bounded by:
...
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering [Figure Placeholder: Use-Case Diagram for CIRCUITRON System showing User, Upload Image, Preprocess, Detect Components, Extract Text/OCR, Generate Schematic, Manual Correction, Convert to Netlist, Run Simulation, Display Results, and AI Assistant with includes and extends relationships]}}
...
In circuit diagrams, values like $10k$, $100\Omega$, $1\mu F$ are crucial. \gls{ocr} must correctly read these values, since even a small error (e.g., mistaking `k' as `1') leads to incorrect simulation behavior. A high character accuracy (target: 0.85 or above) ensures value-to-component mapping is reliable.
...
\begin{itemize}
    \item \textbf{Number of classes:} 60 (excluding ``UNKNOWN'')
...
First, a pairwise analysis was performed on all junction points identified by the object detection model. The algorithm iterates through every unique pair of junctions to calculate the angle of the vector connecting them. Since hand-drawn circuits contain imperfections, an angular tolerance of $\pm 18°$ was applied to classify connections as either horizontal or vertical. To ensure coordinate consistency across the diagram, the resulting segment coordinates were ``snapped'' to a grid using a 16-pixel quantization.
...
\For{all pairs $(j_a, j_b) \in \text{Combinations}(J, 2)$}
    ...
    \If{$|\theta| \le \omega_{\text{tol}} \lor |\theta - 180°| \le \omega_{\text{tol}}$}
    ...
    \ElsIf{$|\theta - 90°| \le \omega_{\text{tol}} \lor |\theta - 270°| \le \omega_{\text{tol}}$}
...
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering [Figure Placeholder: System Flowchart showing Start $\rightarrow$ Authentication $\rightarrow$ Input handwritten image $\rightarrow$ Process image and return editable schematic $\rightarrow$ Update or correction required? $\rightarrow$ Yes: Perform necessary update/correction (loop back) $\rightarrow$ No: Submit for simulation $\rightarrow$ Observe simulation result $\rightarrow$ Need Change? $\rightarrow$ Yes (loop back) $\rightarrow$ No: Save result $\rightarrow$ Stop]}}
...
\begin{itemize}
    \item \textbf{Bounding Boxes (PASCAL VOC format)} for over 61 (including ``UNKNOWN'' class) electrical component classes, including resistors, capacitors, transistors, operational amplifiers, and textual elements.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{rll|rll}
        ...
        5 & gnd & 5,501 & 31 & operational\_amplifier & 752 \\
        6 & vss & 1,354 & 32 & operational\_amplifier.schmitt\_trigger & 16 \\
        7 & voltage.dc & 335 & 33 & optocoupler & 162 \\
        8 & voltage.ac & 292 & 34 & integrated\_circuit & 1,506 \\
        9 & voltage.battery & 765 & 35 & integrated\_circuit.ne555 & 262 \\
        10 & resistor & 14,400 & 36 & integrated\_circuit.voltage\_regulator & 362 \\
        ...
        21 & diode.light\_emitting & 1,212 & 47 & relay & 89 \\
...
\begin{lstlisting}[language=XML, caption={Sample Pascal VOC Annotation Format}, label={lst:pascal-voc-example}]
<annotation>
...
\begin{lstlisting}[caption={Sample LMDB Database Output Format}, label={lst:lmdb-example}]
Key: b'image-00001'
Value: b'\xff\xd8\xff\xe0\x00\x10JFIF...' (JPEG binary data)
