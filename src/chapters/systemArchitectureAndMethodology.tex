...
\textbf{Class 1 Objects (Junction Points):} For junction points, a \textbf{single endpoint} is computed by calculating the bounding box center $(x_c, y_c)$ and snapping to the nearest skeleton pixel within an expanded search region (margin = 8 pixels). The closest skeleton point is returned as the endpoint.

\textbf{Class 2+ Objects (Line Segments):} For line segments, \textbf{two endpoints} are computed using bounding box border intersection. Skeleton pixels that lie on or near the bounding box border (within tolerance) are identified. Among these border hits, true topological endpoints (degree = 1) are preferred. The farthest pair of points is selected as the two endpoints.

\subsection{Graph Construction via Multi-Head BFS}

The Multi-Head BFS algorithm constructs a graph where \textbf{nodes} represent detected endpoints and \textbf{edges} represent skeleton connectivity between endpoints.
